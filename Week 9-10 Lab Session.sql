...
(select `order`.*, customer.cus_gender, customer.cus_name from `order`
 inner join
 customer where `order`.cus_id=customer.cus_id
 having
...
/*6)	Find the least expensive product from each category
 and print the table with 
 category id, name, product name and price of the product*/
 
 select category.*, min(t3.min_price) as Min_Price,t3.pro_name from category
 inner join
...
as t3 where t3.cat_id = category.cat_id
 ...
 INNER JOIN
(SELECT SP.PRICING_ID, SP.PRO_ID FROM supplier_pricing AS SP 
INNER JOIN 
(SELECT PRICING_ID, ORD_DATE FROM `order` where ORD_DATE>"2021-10-05")
 AS T1 ON T1.PRICING_ID=SP.PRICING_ID) 
...
/*8)	Display customer name and gender
 ...
 where cus_name
 like'%A' OR cus_name like'A%';
 
 /*9)	Create a stored procedure to display 
 supplier id, name, rating and Type_of_Service.
...
USE `new-order-directory`;
DROP procedure IF EXISTS `new-order-directory`.`Type_Of_Service`;
;
